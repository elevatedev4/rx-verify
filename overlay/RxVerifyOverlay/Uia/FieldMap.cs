...
public static class FieldMap
{
    ...
    /// <summary>
    /// Leaf-key prefixes for a renewal-response-style refill count, e.g.
    /// "Total fills: 4" or the same verbose parenthetical/multi-colon
    /// style as RefillsKeyPrefix above ("Total fills (...): 4") — seen on
    /// RESPONDED renewal/refill-request e-scripts, where PioneerRx/the
    /// vendor reports the total number of fills (initial fill PLUS
    /// refills) rather than a plain refill count. TWO prefixes (not a
    /// bare "Total fills"), each requiring the immediate delimiter right
    /// after the two words — a naive StartsWith("Total fills") would
    /// over-match any unrelated label that happens to start with those
    /// same two words (e.g. a hypothetical "Total fills authorized this
    /// year: 12"); requiring ": " or " (" immediately after keeps this as
    /// tight as RefillsKeyPrefix's own "Refills (" delimiter. Matched
    /// case-insensitively (real vendor casing hasn't been confirmed
    /// against a live renewal-response dump). EscriptTreeParser.ParseRefills
    /// sets PrescriptionRecord.RefillsFromTotalFills=true when the value
    /// came from one of these keys instead of RefillsKeyPrefix — the raw
    /// value itself passes through UNTRANSFORMED; the engine
    /// (rx-verify src/quantity/index.ts compareRefills) is the single
    /// source of truth for subtracting 1. When BOTH a RefillsKeyPrefix
    /// leaf and one of these are present on the same MedicationPrescribed
    /// node, RefillsKeyPrefix wins — see ParseRefills.
    /// </summary>
    public static readonly string[] TotalFillsKeyPrefixes = { "Total fills: ", "Total fills (" };
